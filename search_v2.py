...
SEARCH_ENGINE_ARGS_SCHEMA = {
    ...
    "properties": {
        ...
        "price_filter": {"title": "price_filter", "description": "Filter by price range (min_price, max_price)", "type": "array", "items": {"type": "number"}},
    },
    "required": [],
}
...
class SearchEngineWithLLM():
    ...
    def construct_se_kwargs(self, kwargs: Dict) -> Dict:
        se_kwargs = {"filters": []}
        if kwargs.get("name"):
            se_kwargs["search_term"] = kwargs["name"]
        ...
        if kwargs.get("popularity_filter"):
            se_kwargs["popularity_filter"].append(PopularityFilter(kwargs["popularity_filter"]))
        if kwargs.get("price_filter"):
            se_kwargs["filters"].append(PriceFilter(kwargs["price_filter"]))
        return se_kwargs
    
    def run(self, input: str) -> str:
        kwargs = self.llm_chain.run(input)
        se_kwargs = self.construct_se_kwargs(kwargs)
        search_results = self.search_engine.search(**se_kwargs)
...
def test():
    ...
    print(seV2.run("Maroon kurta with rating greater than 4 and price less than 400 and ordered by price in descending order"))
